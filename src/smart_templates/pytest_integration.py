...
class SmartPytestTemplates(SmartTemplates):
    ...
    def _prepare_test_context(self, base_context: dict[str, Any]) -> dict[str, Any]:
        ...
        template_context.setdefault("pytest_version", pytest.__version__ if PYTEST_AVAILABLE else "unknown")
    ...
    def generate_test_report(
        ...
        *,
        ...
        try:
            ...
            template_context.setdefault("total_tests", len(test_results.get("tests", [])))
            template_context.setdefault("passed_tests", len([t for t in test_results.get("tests", []) if t.get("status") == "passed"]))
            template_context.setdefault("failed_tests", len([t for t in test_results.get("tests", []) if t.get("status") == "failed"]))
            
            content, error = self.render_safe(template_name, template_context)
            
            if error:
                self._logger.error(f"Failed to render test report template: {error.error.message}")
                return "", error
    ...
    def generate_pytest_fixtures(
        ...
        *,
        ...
        try:
            ...
            if error:
                self._logger.error(f"Failed to render pytest fixtures template: {error.error.message}")
                return "", error
    ...
    def generate_test_cases(
        ...
        *,
        ...
        try:
            ...
            if error:
                self._logger.error(f"Failed to render test cases template: {error.error.message}")
                return "", error
    ...
    def generate_test_documentation(
        ...
        *,
        ...
        try:
            ...
            if error:
                self._logger.error(f"Failed to render test documentation template: {error.error.message}")
                return "", error
    ...
    def generate_mock_objects(
        ...
        *,
        ...
        try:
            ...
            if error:
                self._logger.error(f"Failed to render mock objects template: {error.error.message}")
                return "", error
    ...
    def generate_api_tests(
        ...
        *,
        ...
        try:
            ...
            if error:
                self._logger.error(f"Failed to render API tests template: {error.error.message}")
                return "", error
